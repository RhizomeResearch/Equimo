...
from equimo.timeseries.layers import (
    PatchEncoder,
    ResidualMlp,
    T0Block,
    get_layer,
...
@register_model("t0", modality="timeseries")
class T0(eqx.Module):
    ...
    def _masks(group_ids, variate_type, mask):
        ...
        time_mask = same_doc & ((patch_types == 2)[:, :, None] | causal)
    ...
    def features(
        ...
        *,
        ...
        key_encoder, key_blocks, _ = split_for_mode(key, 3, inference=inference)
        x = self._encode(
            values,
            mask,
            group_ids,
    ...
    def __call__(
        ...
        *,
        ...
        key_encoder, key_blocks, key_decoder = split_for_mode(
            key, 3, inference=inference
        )
        x = self._encode(
            values,
            mask,
...
def _catalog_model_variants():
    ...
    return (
        ModelVariant(
            ...
            notes=(
                "Catalog inputs describe the raw T0 backbone; T0.predict also "
                "provides the upstream scaling and rollout adapter.",
            ),
            field_status=(
                ("inputs", "complete"),
                ("pretrained", "complete"),
